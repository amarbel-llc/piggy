...
- `crates/piggy/src/exec.rs` — `exec_pivy(tool, rest)` (C-pivy handlers + `piggy pivy <tool>` passthrough) + `exec_piggy_ids(subcmd, rest)` (top-level `piggy list`). Formerly `fallback.rs`; renamed for #125 since nothing in it is a temporary bridge. No bash dispatch path survives post-#96.
