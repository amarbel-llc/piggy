...
- `nix/hm/piggy-secrets.nix` — home-manager module `piggy.secrets` (sops-nix-shaped secret manager whose ciphertext is piggy `.ebox` files). Decrypts each `eboxFile` via `piggy box stream decrypt` at `home-manager switch` into a tmpfs generation dir (`$XDG_RUNTIME_DIR/piggy-secrets.d`), `chmod`s to `mode`, then atomically flips the `$XDG_RUNTIME_DIR/piggy-secrets` symlink and prunes old generations. Routes decrypts at piggy-agent via `PIGGY_AUTH_SOCK` (`piggy.agentSocket`). `nixosModules.piggy-secrets` is the thin home-manager re-export (same pattern as `piggy-agent`). Tested by `nix/hm/secrets-eval-test.nix` (`just test-nix-hm-secrets-module`). Design: `docs/plans/2026-06-05-piggy-secrets-nix-module.md`.
