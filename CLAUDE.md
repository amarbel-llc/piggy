...
- **Rust re-impls on the dispatch path** — `piggy agent` and `piggy box` run the Rust re-impls under `crates/piggy/src/cmd/{agent,pivy_box}` (live since the #56–#59 arc closed, 2026-06). `box` falls back to C `pivy-box` for subcommands it doesn't cover yet.
...
- `crates/piggy/src/health.rs` — `piggy health`: 9 fixed read-only/PIN-free agent/card/service checks (split `gather` → pure `evaluate` → `HealthSink`); TAP-14 on a tty / tap-ndjson(7) otherwise. Opt-in `--sign-test` adds an agent self-sign probe (the only path exercising the private key — MAY prompt for a PIN), catching the #179 wedge where an agent enumerates keys but refuses every sign. Design: `docs/plans/2026-06-07-piggy-health-design.md`.
...
**Gate**: `build-go-markl` / `test-go-markl` / `lint-facades` (dagnabit facade-drift check) are wired into the `build`/`test`/`lint` aggregates, so the pre-merge `just` hook exercises the module. Per the #183/#188 ruling there is no `buildGoModule` derivation — go/markl is a consumed library with no piggy-shipped binary; the nix-level gate is the dagnabit facade check.

Recipes: `build-go-markl`, `test-go-markl`, `tidy-go-markl`, `fmt-go-markl`, `codemod-facades` (`dagnabit export`), `lint-facades` (`dagnabit export --check`), `codemod-rfc0002-fixture`.

## Code Conventions

- Bash: `set -o pipefail`, `[[ ]]` conditionals
- Functions: `cmd_*` for user-facing commands, lowercase_with_underscores for helpers
- Formatting is driven by treefmt: shell `shfmt -s -i=2`, nix `nixfmt` (RFC 166), rust `rustfmt`.
- **Always format and check via `just codemod-fmt` (= `nix fmt` = treefmt); never run `cargo fmt` / `rustfmt` / `shfmt` / `nixfmt` bare.** The bare tools use stock defaults that diverge from treefmt's pinned config. `cargo fmt --check` is especially deceptive — it flags unrelated, already-clean files. For a non-mutating check, run `just codemod-fmt` on a clean tree and inspect `git status`.
