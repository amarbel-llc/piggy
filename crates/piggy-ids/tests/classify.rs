...
/// Build a self-signed cert over `key` via openssl, with `cn` as the
/// Subject Common Name, signed with `digest`. Shared boilerplate for
/// the per-algorithm cert builders below.
fn self_signed_cert(
...
fn ed25519_self_signed_cert(cn: &str) -> (Vec<u8>, Vec<u8>) {
    ...
    (
        self_signed_cert(&key, MessageDigest::null(), cn),
...
fn ec_self_signed_cert(nid: openssl::nid::Nid, cn: &str) -> (Vec<u8>, Vec<u8>) {
    ...
    (
        self_signed_cert(&key, MessageDigest::sha256(), cn),
        compressed,
    )
}
