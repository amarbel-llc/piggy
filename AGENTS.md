...
- **exec-to-C passthrough** — `tool` and the generic `piggy pivy <tool>` `exec(2)` into the matching C `pivy-*` binary via `exec::exec_pivy`. Transitional: the C stack is being retired (epic #289, `docs/plans/2026-09-14-retire-c-pivy-rust-nix-migration.md`); the never-built `ca`/`luks`/`zfs` arms are gone (#265). The C-only `agent` features (`-C` confirm, `-K` CAK, `install-service`) stay reachable via `piggy pivy agent`.
...
- `crates/piggy/src/luks.rs` — `piggy luks format|open|add-key|close` (piggy#277): LUKS2 keyed by a store entry's first line, piped to `cryptsetup --key-file -` via `crypt::decrypt_first_line`; `--` forwards cryptsetup args. Bats `t0900-luks.bats`; the LUKS VM lane covers the real path.
