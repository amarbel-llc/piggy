...
| Page | Covers |
|---|---|
| piggy(1) | every `pass` subcommand, `list`, `ssh-copy-id`, `sign-bytes`, `card init`, `health`, `manage`, `secrets reconcile`, `luks`; `PIGGY_*` and `STATSD_*` environment |
...
- **exec-to-C passthrough** — `tool` and `piggy pivy <tool>` exec the C binaries via `exec::exec_pivy`; transitional until #289's Phase 3/5. The never-built `ca`/`luks`/`zfs` arms are gone (#265).
