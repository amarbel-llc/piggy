...
**Grammar export surface (piggy#236)**: piggy is the ROOT of the ecosystem's langlang `@import` chain (piggy → trellis → hyphence; papi consumes validators). The flake exports `.#marklid-grammar` (the peg) and `.#markl-identifier-vectors` (the §7.3 corpus) as store paths so downstreams stage instead of vendoring. The peg's named rules `String`/`Char`/`FormatData`/`Format`/`Data`/`PurposeBare`/`PurposeChar` are a **frozen contract** — renaming any of them fails `TestGrammarImportSurface`, deliberately: that failure means "breaking change for trellis/hyphence/papi", not "update the test". `String`/`Char` are OWNED here (ownership inverted from trellis, which imports them). Producer conformance pattern: RFC 0011 §7.5.
