...
`piggy agent` and `piggy box` run the **Rust** re-impls under `crates/piggy/src/cmd/{agent,pivy_box}`. The Rust `piggy agent` prompts for the PIN on demand via `SSH_ASKPASS` (reusing `card_oracle::run_askpass`), matches guidless piggy 2.x boxes by recipient pubkey in `handle_ecdh_rebox`, and spawns the piggy#244 per-card presence reconcile loop (`cmd/agent/card.rs`) — runtime card hot-swap: each tick it drops a removed card's keys + PIN and adopts a newly-inserted card (sign-gated, #179), replacing the single-primary #59 probe and subsuming the #175 recovery + #143 CAK-swap loops; `--probe-interval` tunes the cadence (default 10s). The re-point is a clean cutover to the Rust flag surface — notably `piggy agent -i` means print-keys-and-exit, NOT C's foreground mode; the home-manager module (`nix/hm/piggy-agent.nix`) emits Rust flags.
