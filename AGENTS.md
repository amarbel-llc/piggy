...
- **Top-level native Rust handlers** — `help` (`usage.rs`), `version` (`version.rs`), `health` (`health.rs`), `ssh-copy-id` (`ssh_copy_id.rs`), `sign-bytes` (`sign_bytes.rs`), `list` (delegates to `piggy-ids`), `card init` (`card/`), `manage` (`manage/`), `secrets reconcile` (`secrets.rs`), `luks` (`luks.rs`).
...
- **exec-to-C passthrough** — `tool` and the generic `piggy pivy <tool>` `exec(2)` into the matching C `pivy-*` binary via `exec::exec_pivy`. Transitional: the C stack is being retired (epic piggy#289, `docs/plans/2026-09-14-retire-c-pivy-rust-nix-migration.md`). The former `ca`/`luks`/`zfs` shortcuts exec'd binaries the nix build never installed and were removed (piggy#265); they return as Rust commands (piggy#277 luks first, #279 zfs, #280 ca). The C-only `agent` features (`-C` confirm, `-K` CAK, `install-service`) stay reachable via `piggy pivy agent`.
...
**Bats lane builder** — `bats.nix` wraps `bats.lib.${system}.batsLane` from the `amarbel-llc/bats` flake. Two scan roots: `zz-tests_bats/t*.bats` and `zz-tests_bats/conformance/*.bats`. `# bats file_tags=` directives are auto-discovered, producing one `bats-<tag>` derivation per tag plus `bats-default`. The default lane filter is `!hardware`: tests tagged `# bats file_tags=hardware` need a real pcscd talking to a card (fibby or hardware) and can't run in the nix sandbox, so they're excluded from `bats-default` and invoked via the `just test-bats-conformance-*-fibby` recipes instead. `zz-tests_bats/explore/` is intentionally not scanned. `nix build .#bats-default` is the authoritative CI gate; the `just test-bats-conformance` recipe is the ergonomic paved path for the non-hardware conformance tests (dual coverage is intentional — piggy#117). fibby (`crates/fibby`) can serve **multiple virtual cards** on one PCSC socket via repeatable `--card NAME [seed-flags…]` groups (piggy#242): each card is its own reader, gets a distinct GUID by default (`--seed-chuid-guid` overrides; `--seed-pin` sets a per-card PIN; `--seed-pin-retries N` starts a card near PIN lockout, piggy#246), and `SCardConnect` routes by reader name — the harness behind the multi-card agent lane (`test-bats-conformance-agent-multicard`). fibby also models **runtime hot-plug** (piggy#130): `--control-socket <path>` opens a second AF_UNIX socket, and `fibby ctl --socket <path> <insert|remove|list> <reader-name>` toggles a card's presence at runtime — the substrate a test drives to exercise the piggy#244 per-card hot-swap lifecycle (`test-bats-conformance-fibby-hotplug` and the hot-swap case in the multi-card lane).

**NixOS VM lane** — `nix/vm-tests/` on igloo's `pkgs.mkVmChecks` + `pkgs.vmTestPrelude` (igloo FDR 0011, `vm-tests(7)`; piggy's lanes were the reference implementation the library was lifted from), exported as `checks.<system>.vm-piggy-{luks,zfs,agent}`, Linux-only (`{ }` elsewhere), **part of the merge gate** via `test` → `_test-vm-linux-only`. A guest runs the shipped `piggy` package with fibby and the Rust `piggy agent` as systemd units (`piggy-stack.nix`, parameterised by card seeds and agent flags, imported per lane via `defaults.imports`), builds a store against the virtual card (`store-bootstrap.nix`), and then: unlocks LUKS2 on `/dev/vdb` from a store secret with a second passphrase keyslot (`luks.nix`); keys a ZFS native-encryption dataset with an unload-key/load-key round trip (`zfs.nix`); and stacks the two agent shapes — the card-backed agent proxying a stock `ssh-agent` upstream (piggy#215) under a `--proxy-only` front (FDR 0001) — against a real `sshd`: login with either key via the front, an `ssh -A` forwarded `pass show`, dead-upstream degradation (`agent.nix`). Every decrypt/sign is asserted against fibby's journal (`GA ECDH 9D` / `GA ECDSA 9A` counts) and the test askpass banner. It is the harness the Rust `piggy luks`/`zfs` ports and the C-pivy retirement are gated on (`docs/plans/2026-09-14-retire-c-pivy-rust-nix-migration.md`). Paved paths: `just test-vm-{luks,zfs,agent}`, `just debug-vm-dry-run <check>` (the kernel-module cache-miss tripwire for the ZFS guest). Runs under TCG on the shared build host (no `/dev/kvm`; its cloud provider exposes no nested virt): 2-3 minutes per lane once the guest closure is cached. `nix flake check` boots them because they are `checks` entries. **Coverage:** `just test-vm-coverage` re-runs the same lanes on a `-C instrument-coverage` build of piggy (`.#piggy-cov`, `.#vm-piggy-*-cov`; each guest ships its `.profraw` files in `$out/coverage`) and `.#coverage-report` (`nix/vm-tests/coverage-report.nix`) merges them with the rustc-matched `llvm-profdata`/`llvm-cov` into `report.txt`, `summary.json` and `lcov.info`. Packages, not checks: outside the merge gate until its wall time is measured. It counts only what the VM lanes exercise, only piggy's own crates, and not fibby (uninstrumented; no SIGTERM handler to flush on).
...
- `crates/piggy/src/health.rs` — `piggy health`: 9 fixed read-only/PIN-free agent/card/service checks (split `gather` → pure `evaluate` → `HealthSink`); TAP-14 on a tty / tap-ndjson(7) otherwise. An agent running with `--upstream` proxying (piggy#215) appends one point per upstream after the base nine, fed by the agent's `upstream-status@piggy` self-report (piggy-private extension; the upstreams themselves are probed with plain `request_identities`, so they implement nothing). A `--proxy-only` agent (per its `agent-mode@piggy` self-report, FDR 0001) SKIPs the four local-card points with a `proxy-only agent` reason, and its upstreams are treated as *alternative* backings — a dead one SKIPs while another is live and FAILs only when none is (card-backed agents keep the additive per-upstream FAIL). Anything short of a positive proxy-only report keeps the card-backed plan. Point 1 (the service check) probes the unit the agent self-reports via `agent-mode@piggy`'s `service` field (`--service-name`, piggy#162) — so multi-instance units (`piggy-agent-<name>.service`) and non-home-manager labels report correctly — falling back to the platform default (`piggy-agent.service` / the launchd label) when the agent doesn't report one. Opt-in `--sign-test` adds an agent self-sign probe (the only path exercising the private key — MAY prompt for a PIN), catching the #179 wedge where an agent enumerates keys but refuses every sign. A dedicated `agent presence: event-driven|poll` point (piggy#248/#255), read from the `agent-mode@piggy` self-report's `event_driven` field, shows the host's presence-detection mode in plain `piggy health` (appended after any upstream points; omitted for a proxy-only or non-self-reporting agent). Design: `docs/plans/2026-06-07-piggy-health-design.md`.
...
- `crates/piggy/src/card/` — the `piggy card init` provisioning stack (piggy#194), structured engine ⟂ interaction-seam ⟂ frontend bindings per RFC 0006. `protocol.rs` is the binding-agnostic `Frontend` trait + serde payload types (which double as the JSON-RPC wire shape). `frontend/tty.rs` is the default in-process binding (card-named askpass prompts); `frontend/jsonrpc.rs` is the JSON-RPC binding an external program drives over `--socket`. `engine.rs` orchestrates full-setup (admin-auth → CHUID → generate 9D+9A → self-signed certs → change PIN+PUK → rotate mgmt key) against `&mut dyn Frontend`. `init_cmd.rs` selects a blank card and drives the engine through the `SessionCard` adapter. Mgmt-key policy: random, displayed once unless sealed — `card/seal.rs` (piggy#258) escrows it into the password store, encrypted to the store's nearest `piggy-ids` (`--seal-management-key[=PASS-NAME]`, default `piv/<GUID>/management-key`; manage's `seal_management_key`; the tty flow offers it when the store can take it). The store/encrypt/git side is the bin's `seal_store.rs`, injected as a `ManagementKeySealer`. The key is sealed before it is set on the card and rolled back if the card rejects it. PIN-protected on-card storage is piggy#198. Bats: `t0950-card-init.bats` + `conformance/piggy_card_init_fibby.bats` (hardware, both frontends).
...
- `crates/piggy/src/store.rs` — `store_root` (`$PIGGY_STORE_DIR > $XDG_DATA_HOME/piggy > $HOME/.local/share/piggy`), `resolve_target` (sneaky-path check), `collect_eboxes`, `find_piggy_ids`, and the pass-name helpers `sneaky_path_reason` / `path_parent_for_search` (shared by `insert` and the #258 seal escrow).
...
- `crates/piggy/src/luks.rs` — `piggy luks format|open|add-key|close` (piggy#277, Phase 3b.1 of the C-pivy retirement): LUKS2 volumes keyed by a store entry's first line, piped to `cryptsetup … --key-file -` via `crypt::decrypt_first_line` (agent path, one card op per call); `--` forwards cryptsetup args. Not a port of C `pivy-luks`; primary keyslots only. Bats: `t0900-luks.bats` (mock cryptsetup); real path: the LUKS VM lane. `piggy zfs` (#279) reuses `decrypt_first_line`.
- `crates/piggy/src/secrets.rs` — `piggy secrets reconcile` (FDR 0003): materialises ebox-backed secrets as plain files from a JSON manifest. Offline classification (ciphertext SHA-256 + target stat fingerprint in `$XDG_STATE_HOME/piggy/secrets/state.json`, never mtime), one-PIN batch decrypt via the shared `show_batch::with_batch_unlock` backend (card first, agent fallback), atomic rename writes, explicit ownership (unrecorded target = conflict unless `adopt`), never deletes. TAP-14. Bats: `conformance/piggy_secrets_reconcile_fibby.bats` (hardware).
- `nix/hm/piggy-secrets.nix` — `homeManagerModules.piggy-secrets` (`services.piggy-secrets`, NixOS re-export in `nix/nixos/`). Ciphertext copies + manifest are store paths named `piggy-secrets-*` referenced WITHOUT string context, so they stay out of the home generation's closure; activation GC-roots them under `$XDG_STATE_HOME/piggy/secrets/`, runs the offline `--check`, and `--no-block` starts the oneshot `piggy-secrets.service` on drift — never decrypts during a switch, never fails it. Eval test: `nix/hm/secrets-eval-test.nix` (`just test-nix-hm-secrets-module`).
...
`piggy agent` and `piggy box` run the **Rust** re-impls under `crates/piggy/src/cmd/{agent,pivy_box}`. The Rust `piggy agent` prompts for the PIN on demand via `SSH_ASKPASS` (reusing `card_oracle::run_askpass`), matches guidless piggy 2.x boxes by recipient pubkey in `handle_ecdh_rebox`, and spawns the piggy#244 per-card presence reconcile loop (`cmd/agent/card.rs`) — runtime card hot-swap: each tick it drops a removed card's keys + PIN and adopts a newly-inserted card (sign-gated, #179), replacing the single-primary #59 probe and subsuming the #175 recovery + #143 CAK-swap loops; `--probe-interval` tunes the cadence (default 10s). While the agent serves 0 keys for the first ~30s after launch it polls fast (~1s, `next_reconcile_delay`, piggy#251) so a card contended by an outgoing agent during a deploy is adopted within ~1s of release, not up to a full interval later. Event-driven presence is the DEFAULT (piggy#248, made default in piggy#255; `--poll-only` opts out): a dedicated thread (`card::run_event_source`) watches PC/SC reader states via `SCardGetStatusChange`, records which readers transitioned (`State::CHANGED`), and fires a `Notify`, so `reconcile_loop_with_events` runs an immediate pass that collapses the debounce to one miss for ONLY those readers' cards (a card whose reader didn't change keeps its full blip debounce, so an event about one reader never evicts a blipping other card) — the `--probe-interval` poll stays the safety net. `--poll-only` reconciles by polling only (`reconcile_loop`); both flags conflict with `--proxy-only` (no card loop). The `agent-mode@piggy` self-report's `event_driven` field carries the active mode, surfaced as `piggy health`'s `agent presence: …` point. fibby drives the event path in tests: its `WAIT_READER_STATE_CHANGE` (0x13) registers + replies the reader-state array (old-mode `protocol<=4005`), a `fibby ctl` toggle delivers the 8-byte async wake, and a per-reader `event_counter` advances. The re-point is a clean cutover to the Rust flag surface — notably `piggy agent -i` means print-keys-and-exit, NOT C's foreground mode; the home-manager module (`nix/hm/piggy-agent.nix`) emits Rust flags.
