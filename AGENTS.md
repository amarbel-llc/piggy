...
**NixOS VM lane** — `nix/vm-tests/` (`pkgs.testers.runNixOSTest`, exported as `checks.<system>.vm-piggy-{luks,zfs}`, Linux-only). A guest runs the shipped `piggy` package with fibby and the Rust `piggy agent` as systemd units (`piggy-stack.nix`), builds a store against the virtual card, and unlocks real block-level encryption from a store secret: LUKS2 on `/dev/vdb` with a second passphrase keyslot (`luks.nix`) and a ZFS native-encryption dataset with an unload-key/load-key round trip (`zfs.nix`). It is the harness the Rust `piggy luks`/`zfs` ports and the C-pivy retirement are gated on (`docs/plans/2026-09-14-retire-c-pivy-rust-nix-migration.md`). Paved paths: `just test-vm-luks`, `just test-vm-zfs`, `just debug-vm-dry-run <check>` (the kernel-module cache-miss tripwire for the ZFS guest). Runs under TCG on flac (no `/dev/kvm`; Hetzner exposes no nested virt), so it lives under `test-optional`, not the pre-merge gate; `nix flake check` boots it because it is a `checks` entry.
