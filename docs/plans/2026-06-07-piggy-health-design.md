...
| # | point | semantics |
|---|---|---|
| 1 | `service: piggy-agent.service active` | `systemctl --user show piggy-agent.service --property=LoadState,ActiveState,SubState,ExecMainStatus`. ok = active; not ok = unit exists but inactive/failed (diag: the four properties); SKIP = non-Linux, no systemctl, or `LoadState=not-found` (manual agent setups stay green). |
